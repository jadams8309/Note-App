...
const cancelEdit = (noteDiv) => {
    ...
    const bodyP = noteDiv.querySelector('button.edit');
    bodyP.editButton = false;
    const editButton = noteDiv.querySelector('button.edit');
    editButton.innterHTML = 'Edit Note';
...
const editNote = (noteDiv, editSave = false) => {
    ...
    if (editSave) {
        const note = notes.find(note.id == noteDiv.id);
        ...
        editButoon.onclick = () => editNote(noteDiv)
        deleteButton.onclick = ()  => deleteNote(noteDiv);
    }
}
const saveNote = () => {
    ...
    const bodyColorInput = document.querySelector('select');
